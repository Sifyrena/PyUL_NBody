...
def evolve(save_path,run_folder, EdgeClear = False, DumpInit = False, DumpFinal = False, UseInit = False, IsoP = False, UseDispSponge = False, SelfGravity = True, NBodyInterp = True, NBodyGravity = True, Shift = False, Simpson = False, Silent = False, AutoStop = False, AutoStop2 = False,AutoStop3 = False, KEThreshold = 0.9, WellThreshold = 100, InitPath = '', InitWeight = 1, Message = '', Stream = False, StreamChar = [0], SphExpand = False, Sph_Max_n = 9, Sph_Only_Sym = True):
    ...
    PyULConfig["Spherical Harmonic Expansion (Experimental)"] = ({
    "Perform Expansion": SphExpand,
    "Maximum Principle Order": Sph_Max_n,
    "Only m = 0": Sph_Only_Sym,      
    })
    ...
    ##########################################################################################
    # SET UP THE SPH HARMONICS IN RAM
    
    if SphExpand:
        
        mSC = convert(solitons[0][0],s_mass_unit,'m')
        
        Basis = NLMCompile(resol, mSC, xarray,yarray,zarray,max_n = Sph_Max_n, OnlySym = Sph_Only_Sym)
    ...
    # THIS WAS MOVED FROM ABOVE Search for [->]
    psi = ne.evaluate("exp(1j*(velx*xarray + vely*yarray + velz*zarray))*psi")
    ...
    if SphExpand:
        ExpansionCoefficientPrep(psi,Basis,loc,-1,1)
...
# SAVE DESIRED OUTPUTS
        if ((ix + 1) % its_per_save) == 0:
            ...
            if SphExpand:
                ExpansionCoefficientPrep(psi,Basis,loc,ix,its_per_save)
...
# These go into PyUL Main Pipeline. Assumes Code Units in Everything
...
def HighBasisInit_O(n,resol,mS, xarray, yarray, zarray):
    ...
    Name = f'./Soliton Profile Files/HFK/f_STD_{n:02d}'
    ...
    for n in range(max_n):
        ...
        for l in range(n+1):
            ...
            for m in range(mrange):
                ...
                bases[nlmString] = np.conj(RadialFN*Ylm)
...
def ExpansionCoefficientPrep(psi,Basis,loc,ix,its_per_save):
    ...
    for key, value in Basis.items():
        ...
        Result[key] = np.abs(np.sum(psi*value))
