...
def initsoliton(funct, xarray, yarray, zarray, position, alpha, f, delta_x):
    for index in np.ndindex(funct.shape):
        ...
        if (np.sqrt(alpha) * distfromcentre <= 5.6):
            ...
            funct[index] = alpha * f[int(np.sqrt(alpha) * (distfromcentre / delta_x + 0))]
        else:
            funct[index] = 0
    return funct
...
def evolve(central_mass, num_threads, length, length_units, resol, duration, duration_units, step_factor, save_number, save_options,
    ...
    distarray = ne.evaluate("(xarray**2+(yarray-length/2)**2+zarray**2)**0.5") # Radial coordinates
    ...
    print("The preamble took " + str(round(end-start, 2)) + " seconds.")

    start = time.time()


    ##########################################################################################
    # SETUP PADDED POTENTIAL HERE
    ...
    print("Potential setup took " + str(round(end-start, 2)) + " seconds.")
    ...
    print("The loop took " + str(round(end-start, 2)) + " seconds.")
    ################################################################################
    # LOOP ENDS
