...
def evolve(central_mass, num_threads, length, length_units, 
    ...
    RANDOM_BG = np.random.rand(resol,resol,resol)*0
    RANDOM_PHASE = np.random.rand(resol,resol,resol)*2*np.pi
    
    psi = ne.evaluate("psi + RANDOM_BG*exp(1j*RANDOM_PHASE)")
    
    print('PyUL_NBody: Adding initial noise in wavefunction. Maximum amplitude 0.')
