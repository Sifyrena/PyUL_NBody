...
# Codename 000.01 - 000.14
# This Archive: 10am, 2020/Sept/28
...
def D_version():
    return str('2020 10 08, With Mirror Images and New Energy Fix')
...
def calculate_energiesF(save_options, resol,
        ...
        fft_psi, ifft_funct,
        egpcmlist, egpsilist, ekandqlist, egylist, mtotlist,xarray, yarray, zarray, a, Images, ImMatrix,gridlength):
    ...
    if (save_options[3]):
            ...
            for MI in range(len(masslist)):
                ...
                if Images:
                    ...
                    for ImInd in range(len(ImMatrix)):
            
                        ImShift = ImMatrix[ImInd]
                    
                        TMx = State[0] + gridlength*ImShift[0]
                        TMy = State[1] + gridlength*ImShift[1]
                        TMz = State[2] + gridlength*ImShift[2]
                        ...
                        mPhi = ne.evaluate("mPhi-a*TMmass/(a*distarrayTM+exp(-a*distarrayTM))") # Potential Due to Test Mass
...
def FWNBody(t,TMState,masslist,FieldFT,gridlength,resol,Kx,Ky,Kz,epsilon):
...
def evolve(central_mass, num_threads, length, length_units, 
    ...
    Infini = False
    Images = True # Creates 6 Images For Each Mass Doing Stuff in the Box
    ...
    # These are the 6 Immediate Boxes In the Periodic BC.
    ...
    for particle in particles:
        ...
        if Images:
            ...
            print('PyUL NBody: For Each Mass Entered, %.0f Images Were Created' % len(ImMatrix))
            ...
            for ImInd in range(len(ImMatrix)):
                ...
                TMx = position[0] + gridlength*ImShift[0]
                TMy = position[1] + gridlength*ImShift[1]
                TMz = position[2] + gridlength*ImShift[2]
    ...
    for ix in range(actual_num_steps):
        ...
        if (((ix + 1) % its_per_save) == 0) and halfstepornot == 0:
            ...
            if (save_options[3]):
                calculate_energiesF(save_options, resol, psi, cmass, TMState, masslist, Vcell, phisp, karray2, funct,fft_psi, ifft_funct,egpcmlist, egpsilist, ekandqlist, egylist, mtotlist,xarray, yarray, zarray, a, Images, ImMatrix,gridlength)
