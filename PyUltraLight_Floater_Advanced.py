...
def prog_bar(iteration_number, progress, tinterval,TMState):
    ...
    text = "\r[{}] {:.0f}% {}{}{}{}".format(
        "良" * block + "艮" * (size - block), round(progress * 100, 0),
    ...
    for i in range(len(masslist)):
        ...
        for ii in range(len(masslist)):
            ...
            if ii != i:
                ...
                #YDDOT
                dTMdt[Ind+4] = dTMdt[Ind+4] + masslist[ii]/rCube*rV[1]
                #ZDDOT
                dTMdt[Ind+5] = dTMdt[Ind+5] + masslist[ii]/rCube*rV[2]
...
def evolveF(num_threads, length, length_units, resol, 
    ...
    clear_output()
    print("PyUL_NBody: FW Modified Version. August MMXX. \n")
    ...
    if NS == 0:
        ...
        print("PyUL_NBody: Advancing NBody Simulation Using Explicit RK4.\n")
    else:  
        print("PyUL_NBody: Advancing NBody Simulation Using RK23 with at Least %.0f steps per ULD iteration.\n" % NS)
    ...
    for ix in range(actual_num_steps):
        ...
        if Method == 2 and NumSol != 0:
            ...
            TMxGrid = ne.evaluate("1j*Kx*FieldFT")
            TMyGrid = ne.evaluate("1j*Ky*FieldFT")
            TMzGrid = ne.evaluate("1j*Kz*FieldFT")
            
            irfft_TMG = pyfftw.builders.ifftn(TMxGrid, axes=(0, 1, 2), threads=num_threads)
        
            Gx = np.real(irfft_TMG(TMxGrid))
            Gy = np.real(irfft_TMG(TMyGrid))
            Gz = np.real(irfft_TMG(TMzGrid))
            
            GxI = RegularGridInterpolator((SR,SR,SR),Gx,method='linear',bounds_error = False, fill_value = 0)
