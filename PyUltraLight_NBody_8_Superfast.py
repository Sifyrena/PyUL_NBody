...
def save_grid(
        ...
        if (save_options[9]): # Temporarily Appropriated!
            ...
            if (npy):
                file_name = "Arg2D_#{0}.npy".format(save_num)
                np.save(
                    os.path.join(os.path.expanduser(loc), file_name),
                    psiarg
                )
            if (npz):
                file_name = "Arg2D_#{0}.npz".format(save_num)
                np.savez(
                    os.path.join(os.path.expanduser(loc), file_name),
                    psiarg
                )
            if (hdf5):
                file_name = "Arg2D_#{0}.hdf5".format(save_num)
                file_name = os.path.join(os.path.expanduser(loc), file_name)
                f = h5py.File(file_name, 'w')
...
def evolve(save_path,run_folder,Method):
    ...
    print(f"\n{Version} Runtime: Run Complete. Time Elapsed (d:h:m:s): {day:.0f}:{hour:.0f}:{minutes:.0f}:{seconds:.2f}")
