Version   = str('PyUL2QW') # Handle used in console.
D_version = str('Integrator Build 2021 03 18: Quantum Wind Edition') # Detailed Version
S_version = 17.5
 # Short Version
...
def evolve(save_path,run_folder, Method = 3, Draft = True, EdgeClear = False, DumpInit = False, IsoP = False, DynDrag = False, UseDispSponge = False, NonVerbose = False, CuOfPo = 1/4):
    ...
    CuOfRe = int((1-CuOfPo)*resol)
    
    print('\n\n\n\n')
    for i in range(CuOfRe,resol,1):
        print(f'{i}',end = '  -  ')
    ...
    if Uniform:
        ...
        psi = ne.evaluate("exp(1j*(vely*yarray))*psi")
        ...
        if DumpInit:
            ...
            f = h5py.File(file_name, 'w')
            dset = f.create_dataset("init", data=psiInit)
            f.close()

        print('===========================ULDM WIND INFORMATION==============================')
        print(f"{Version} Init: The plane wave along the y direction is at {vely:.4g} code speed.")
        print('==============================================================================')

                
    else:
        raise RuntimeError('No wind to blow! Are you looking for the general-purpose PyUL?')
    ...
    for ix in range(actual_num_steps):
        ...
        psiInit = ne.evaluate("exp(1j*(-0.5*(vely*vely)*h))*psiInit")
        ...
        # psi[:,CuOfRe:resol,:] = psiInit[:,CuOfRe:resol,:]
...
def NBodyEnergy(MassListSI,TMDataSI,EndNum,a=0,length_unit = ''): # kg, m, m/s, Int, code unit -1
    
    if a == 0:
        print('Warning: Using Standard Newtonian Potential.')
    
    else:
        ADim = convert(a,length_unit,'l')
...
def SolitonSizeEstimate(mass,length,resol,mass_unit = '',length_unit = ''): # Only deals with default solitons!
    import matplotlib.pyplot as plt
    
    code_mass = convert(mass,mass_unit,'m')
    code_length = convert(length,length_unit,'l')
